...
<style>
    ...
    .price-target { color: #27ae60; font-size: 18px; font-weight: bold; }
...
def salvar_no_db(novo_dict):
    try:
        sheet = conectar_gsheets()
        # Garante que os valores numéricos sejam salvos como float/string limpa
        linha = [
            novo_dict['Data'],
            novo_dict['Ticker'],
            float(novo_dict['Preço Justo']), # Força float
            float(novo_dict['Cotação Ref']), # Força float
            novo_dict['Método'],
            novo_dict['Tese'],
...
if opcao == "🏠 Início":
    ...
    with st.container(border=True):
        ...
        c3, c4 = st.columns(2)
        # --- BLINDAGEM NOS CAMPOS DE ENTRADA ---
        ...
        if st.button("💾 SALVAR ESTUDO", type="primary"):
            if f_ticker:
                ...
                try:
                    val_justo_final = float(f_justo.replace("R$", "").replace(",", ".").strip())
                    val_cotacao_final = float(f_cotacao.replace("R$", "").replace(",", ".").strip())
                except:
                    st.error("Erro no valor do preço. Use ponto ou vírgula (Ex: 37.50).")
                    st.stop()
    ...
    if lista_db:
        for item in lista_db[::-1]:
            try:
                if isinstance(item['Premissas_JSON'], str):
            ...
            with st.container(border=True):
                ...
                col_head1.subheader(f"📊 {item['Ticker']} | {item['Metodo']}")
                col_head2.caption(item['Data'])
                st.divider()
